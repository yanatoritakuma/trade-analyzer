...
```
【株価取得】平日 15:00 JST            【分析実行】平日 15:30 JST
        ↓                                    ↓
AWS EventBridge                      AWS EventBridge
        ↓                                    ↓ 直接invoke {"job":"analyze"}（案B・IAM認証）
AWS Lambda（Python）                  AWS Lambda（Go Worker）
yfinanceで株価取得                    stock_prices+watchlistを読み Claude APIに分析依頼
        ↓ HTTP POST                          ↓
Go / Gin（/internal/stock-prices）    ┌──────────────────────────┐
stock_prices を UPSERT                ↓                          ↓
                                Neon PostgreSQL            LINE Messaging API
                              （分析結果・履歴を保存）   （BUY/SELLシグナル通知）
```

> **採用構成（案B）**: 分析実行は API Gateway/HTTP を介さず **EventBridge → Go Worker Lambda の直接invoke**
> で行う（`{"job":"analyze"}`）。Python Lambda は 15:00 に株価取得して `stock_prices` を更新するのみで、
> 分析は 15:30 に Worker Lambda が `stock_prices` を読んで実行する。詳細は
> `doc/dev-spec/infra_architecture.md` / `infra_requirements.md` を参照。
...
毎週日曜 18:00
     ↓
AWS EventBridge
     ↓ 直接invoke {"job":"weekly_report"}（案B・IAM認証）
AWS Lambda（Go Worker・RunWeekly）
     ↓
...
| POST | `/internal/stock-prices` | 株価データ受取・stock_prices保存（UPSERT） |
| ~~POST~~ | ~~`/internal/weekly-report`~~ | **廃止/任意**: 案Bにより週次は EventBridge→Worker Lambda直接invokeで起動（HTTP不要）。手動トリガー用に残すのは任意 |

> **案B採用に伴う変更**: 分析実行・週次レポートの**起動**はHTTPエンドポイントではなく
> **EventBridge → Worker Lambda の直接invoke**（`{"job":"analyze"}` / `{"job":"weekly_report"}`）で行う。
> `/internal/stock-prices` は引き続き Python 株価取得Lambda が `stock_prices` を更新する用途で使用する
> （分析の起動はこのPOSTでは行わず、15:30の Worker Lambda が担う）。
...
#### 週次レポート起動（案B：HTTPではなくEventBridge直接invoke）

案Bにより、週次レポートの起動は HTTP ではなく EventBridge → Worker Lambda の直接invokeで行う。

```json
// EventBridge ルールが渡す定数input（Worker Lambda の dispatch が判定）
{ "job": "weekly_report" }
```

`dispatch` が `ReportUsecase.RunWeekly(ctx)` を呼び、当週集計→Claude学習→`learning_logs`/`learning_versions`
保存→S3アップロードを行う（パイプライン本体は別フェーズ実装）。
HTTPの `/internal/weekly-report` を手動トリガー用に残す場合のみ、上記JSON相当を `InternalAuth` 付きで実装する。
...
| 処理 | Cron（UTC・CDK実装） | JST | ターゲット／方式 |
|------|----------------------|-----|------------------|
| 株価取得 | `0 6 ? * MON-FRI *` | 平日 15:00 | Python株価取得Lambda（別フェーズ・ARN指定で有効化） |
| 分析実行 | `30 6 ? * MON-FRI *` | 平日 15:30 | **Go Worker Lambda 直接invoke**（`{"job":"analyze"}`・案B） |
| 週次レポート | `0 9 ? * SUN *` | 日曜 18:00 | **Go Worker Lambda 直接invoke**（`{"job":"weekly_report"}`・案B） |

> 実装は `infra/lib/trade-analyzer-stack.ts`（CDK）。分析/週次は API Gateway を介さず Worker Lambda を
> 直接invoke する（案B：Connection不要でコスト削減・30秒制限回避）。株価取得は15:00に分離し、
> 分析(15:30)は更新済み `stock_prices` を読む。
