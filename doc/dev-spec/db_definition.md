...
| name | VARCHAR(100) | NULL | - | 銘柄名（記録時に `watchlist.name` から非正規化保存。履歴表示用） |
...
`{期間}_{方向}` の組み合わせ。UI（SCR-12）の「期間（短期/中期）×方向（順張り/逆張り/両方）」を一意にマップできるよう6値を定義する。

| 値 | 期間 | 方向 |
|----|------|------|
| `short_term_trend` | 短期 | 順張り |
| `short_term_contrarian` | 短期 | 逆張り |
| `short_term_both` | 短期 | 両方 |
...
CREATE TABLE trades (
  ...
  ticker       VARCHAR(10)   NOT NULL,
  name         VARCHAR(100),                   -- 記録時にwatchlist.nameから保存（履歴表示用）
  mode         VARCHAR(10)   NOT NULL
